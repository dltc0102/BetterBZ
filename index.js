import { isInHypixel, replaceBazaarMessage, stripRomanNumerals, truncateNumbers } from './functions.js';
...
const soundCommandClickable = new TextComponent(`&8[&r&e&lTURN SOUNDS &r${bzData.sounds ? '&c&lOFF&r' : '&a&lON&r'}&8]`)
    .setClick('run_command', '/bzsounds')
    .setHover('show_text', '/bzsounds');

register('command', () => {
    if (!bzData.sounds) {
        bzData.sounds = true;
        ChatLib.chat(`&6[BetterBZ] &rSounds: &a&lON&r`);
    } else if (bzData.sounds) {
        bzData.sounds = false;
        ChatLib.chat(`&6[BetterBZ] &rSounds: &c&lOFF&r`);
    }
}).setName('bzsounds');

register('gameLoad', () => {
    const soundStatus = bzData.sounds ? '&a&lON&r' : '&c&lOFF&r';
    const bzLoadMessage = `&6[BetterBZ] &7Loaded! &3[&rSounds: ${soundStatus}&3]`;
    const supportMessage = new Message (
        `${bzLoadMessage} &r&8-- `, soundCommandClickable
    );
...
const bzErrorTypes = {
    "You have goods to claim on this order!": "You have goods to claim on this order!",
    ...
    "You don't have anything to sell!": "Nothing to sell!",
    "There are no Buy Orders for this product!": "There are no Buy Orders for this product!",
    "No items could be matched to buyers!": "No items could be matched to buyers!",
    "Couldn't match the quoted price!": "Product couldn't match the quoted price!",
    "The Sell Offers for this item changed too much!": "This product's sell offers are too volatile!",
};
